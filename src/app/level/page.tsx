...
const Page = () => {
  ...
  const colorCombos = [
    ...
    },
    {
      ...
      btn: "bg-yellow-500 text-black hover:bg-yellow-600",
    },
    {
    ...
    },
    {
      ...
      btn: "bg-cyan-500 text-black hover:bg-cyan-600",
    },
  ];
    ...
    const fetchAllPkg = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/users/get-packages`
        );
    ...
    <div className="bg-black min-h-screen flex flex-col items-center py-10 px-4 mb-8 relative">
      ...
      >
        {packages.map((pkg, index) => (
          ...
          >
            <div
              className={`bg-gradient-to-br ${pkg.bgColor} relative rounded-2xl`}
            >
              {/* VIP Badge Image */}
              {pkg.isVip && (
                <motion.div
              ...
              <div className="px-6 pt-16 pb-4 text-white">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <p className="text-xl font-semibold">
                    {pkg.deposit.toLocaleString()} NPR
                  </p>
                </div>
                ...
                <div className="flex justify-between bg-white text-black rounded-lg px-4 py-2 mb-4 shadow-inner">
                  <p>
                    Per Task:{" "}
                    <span className="font-semibold">₹{pkg.perTask}</span>
                  </p>
                  <p>
                    Daily Task:{" "}
                    <span className="font-semibold">{pkg.dailyTask}</span>
                  </p>
                </div>
